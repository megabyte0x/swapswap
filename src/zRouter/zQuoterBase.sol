...
//     {
//         unchecked {
//             if (swapAmount == 0) return (best, new Quote[](0));
//             // 1 (V2) + 1 (AERO) + 4 (ZAMM) + 4 (V3) + 4 (V4) + 4 (AERO_CL)
//             quotes = new Quote[](18);
...
//             // --- V2 ---
//             (amountIn, amountOut) = quoteV2(exactOut, tokenIn, tokenOut, swapAmount);
//             quotes[j++] = Quote(AMM.UNI_V2, 30, amountIn, amountOut);

//             // --- AERO (stable/volatile best-of) ---
//             {
//                 uint256 bps;
//                 (amountIn, amountOut, bps) = quoteAero(exactOut, tokenIn, tokenOut, swapAmount);
//                 quotes[j++] = Quote(AMM.AERO, uint16(bps), amountIn, amountOut);
//             }

//             // --- ZAMM (4 tiers) ---
//             (amountIn, amountOut) = quoteZAMM(exactOut, 1, tokenIn, tokenOut, 0, 0, swapAmount);
//             quotes[j++] = Quote(AMM.ZAMM, 1, amountIn, amountOut);
//             (amountIn, amountOut) = quoteZAMM(exactOut, 5, tokenIn, tokenOut, 0, 0, swapAmount);
//             quotes[j++] = Quote(AMM.ZAMM, 5, amountIn, amountOut);
//             (amountIn, amountOut) = quoteZAMM(exactOut, 30, tokenIn, tokenOut, 0, 0, swapAmount);
//             quotes[j++] = Quote(AMM.ZAMM, 30, amountIn, amountOut);
//             (amountIn, amountOut) = quoteZAMM(exactOut, 100, tokenIn, tokenOut, 0, 0, swapAmount);
//             quotes[j++] = Quote(AMM.ZAMM, 100, amountIn, amountOut);

//             // --- Uni v3 single-tick ---
//             uint24[4] memory fees = [uint24(100), uint24(500), uint24(3000), uint24(10000)];
//             for (uint256 i; i != fees.length; ++i) {
//                 (amountIn, amountOut) = quoteV3(exactOut, tokenIn, tokenOut, fees[i], swapAmount);
//                 quotes[j++] = Quote(AMM.UNI_V3, uint16(fees[i] / 100), amountIn, amountOut); // 1/5/30/100 bps
//             }
...
//             // --- Aerodrome CL single-tick — mirror v3 fee tiers ---
//             {
//                 int24[4] memory tickSpaces = [int24(1), int24(10), int24(60), int24(200)];
//                 for (uint256 i; i != tickSpaces.length; ++i) {
//                     (amountIn, amountOut) = quoteAeroCL(exactOut, tokenIn, tokenOut, tickSpaces[i], swapAmount);
//                     quotes[j++] = Quote(AMM.AERO_CL, uint16(fees[i] / 100), amountIn, amountOut); // 1/5/30/100 bps
//                 }
//             }
...
//                 // skip quotes that are unavailable for the chosen mode
//                 if ((!exactOut && q.amountOut == 0) || (exactOut && q.amountIn == 0)) continue;
...
//                     V4TickBitmap.nextInitializedTickWithinOneWord(V4_STATE_VIEW, poolId, tick, tickSpacing, zeroForOne);
...
//             // treat ETH as WETH to match router behavior:
//             if (tokenIn == address(0)) tokenIn = WETH;
//             if (tokenOut == address(0)) tokenOut = WETH;

//             // Try volatile first, then stable:
//             (uint256 aiV, uint256 aoV) = _quoteAeroPool(exactOut, /*stable=*/ false, tokenIn, tokenOut, swapAmount);
//             (uint256 aiS, uint256 aoS) = _quoteAeroPool(exactOut, /*stable=*/ true, tokenIn, tokenOut, swapAmount);

//             // pick the better of the two:
//             if (!exactOut) {
//                 // maximize amountOut, break ties on lower amountIn
//                 bool takeV = (aoV > aoS) || (aoV == aoS && aiV != 0 && (aiS == 0 || aiV < aiS));
//                 if (takeV && aoV != 0) return (aiV, aoV, 20);
//                 if (aoS != 0) return (aiS, aoS, 2);
//             } else {
//                 // minimize amountIn, break ties on higher amountOut
//                 if ((aiV != 0 && (aiS == 0 || aiV < aiS)) || (aiV != 0 && aiV == aiS && aoV > aoS)) {
//                     return (aiV, aoV, 20);
//                 }
//                 if (aiS != 0) return (aiS, aoS, 2);
//             }
//             return (0, 0, 0);
//         }
//     }
...
//     {
//         unchecked {
//             (address pool, /*zeroForOne*/ ) = _aeroPoolFor(tokenIn, tokenOut, stable);
//             if (!_isContract(pool)) return (0, 0);

//             if (!exactOut) {
//                 amountIn = swapAmount;
//                 amountOut = IAeroPool(pool).getAmountOut(amountIn, tokenIn);
//                 // pool returns 0 if unavailable; keep zeros so caller can compare
//                 return (amountIn, amountOut);
//             } else {
//                 amountOut = swapAmount; // target out
//                 // Solve for minimal input using monotonicity of getAmountOut(amountIn)
//                 amountIn = _aeroSolveExactOut(pool, tokenIn, amountOut);
...
//     // Monotone solver for exactOut on Aerodrome classic pools.
//     // Doubles the upper bound until it covers target, then binary-searches to 1 wei.
//     function _aeroSolveExactOut(address pool, address tokenIn, uint256 targetOut) internal view returns (uint256) {
//         unchecked {
...
//             // exponential search for an upper bound (<= 2^64 steps worst-case cap):
//             for (uint256 i; i != 64; ++i) {
//                 uint256 outHi = IAeroPool(pool).getAmountOut(hi, tokenIn);
//                 if (outHi >= targetOut && outHi != 0) break;
//                 // if outHi == 0 we still increase hi (pool may be stable curve near flat segment)
//                 hi <<= 1;
//                 if (hi == 0) return 0; // overflow guard
//             }

//             // if even huge hi didn't reach target, give up
//             if (IAeroPool(pool).getAmountOut(hi, tokenIn) < targetOut) return 0;

//             // binary search for minimal input that yields >= targetOut
//             for (uint256 i; i != 32; ++i) {
//                 uint256 mid = (lo + hi) >> 1;
//                 uint256 outMid = IAeroPool(pool).getAmountOut(mid, tokenIn);
//                 if (outMid >= targetOut) {
//                     hi = mid;
//                 } else {
//                     lo = mid + 1;
...
//             }
//             // final check
//             return IAeroPool(pool).getAmountOut(lo, tokenIn) >= targetOut ? lo : 0;
//         }
//     }
...
//             // Treat native as WETH to match router behavior
//             address tIn = tokenIn == address(0) ? WETH : tokenIn;
//             address tOut = tokenOut == address(0) ? WETH : tokenOut;

//             (address pool, bool zeroForOne) = _aeroCLPoolFor(tIn, tOut, spacing);
//             if (pool == address(0)) return (0, 0);

//             // Fee in micro-pips (1e-6); prefer spacing map, fall back to per-pool
//             uint24 mapped = IAeroCLFactory(AERO_CL_FACTORY).tickSpacingToFee(spacing);
//             uint24 feeMicro = mapped != 0 ? mapped : IAeroCLFactory(AERO_CL_FACTORY).getSwapFee(pool);
//             if (feeMicro >= 1_000_000) return (0, 0); // defensive: disallow 100%+

//             // Open price limits (same ±1 convention as v3)
//             uint160 sqrtPriceLimitX96 = zeroForOne ? MIN_SQRT_RATIO_PLUS_ONE : MAX_SQRT_RATIO_MINUS_ONE;

//             // Read state
//             IAeroV3Pool p = IAeroV3Pool(pool);
//             (uint160 sqrtP, int24 tick, /*obsIndex*/, /*obsCard*/, /*obsCardNext*/, /*unlocked*/ ) = p.slot0();
//             uint128 L = p.liquidity();
//             if (sqrtP == 0 || L == 0) return (0, 0);

//             // v4-style sign convention for reuse of SwapMath:
//             // exact-in  => amountRemaining < 0
//             // exact-out => amountRemaining > 0
//             int256 amountRemaining = exactOut ? int256(swapAmount) : -int256(swapAmount);
//             int256 amountCalculated;

//             while (amountRemaining != 0 && sqrtP != sqrtPriceLimitX96) {
//                 (int24 tickNext, bool initialized) =
//                     AeroCLTickBitmap.nextInitializedTickWithinOneWord(p, tick, spacing, zeroForOne);

//                 // clamp within TickMath bounds
//                 if (tickNext < TickMath.MIN_TICK) tickNext = TickMath.MIN_TICK;
//                 else if (tickNext > TickMath.MAX_TICK) tickNext = TickMath.MAX_TICK;

//                 uint160 sqrtPriceNextX96 = TickMath.getSqrtPriceAtTick(tickNext);

//                 // step toward next initialized tick or the price limit
//                 (uint160 sqrtPriceNext, uint256 stepIn, uint256 stepOut, uint256 feeAmt) = SwapMath.computeSwapStep(
//                     sqrtP,
//                     (zeroForOne ? (sqrtPriceNextX96 < sqrtPriceLimitX96) : (sqrtPriceNextX96 > sqrtPriceLimitX96))
//                         ? sqrtPriceLimitX96
...
//                     L,
//                     amountRemaining,
//                     feeMicro // micro-pips, matches MAX_SWAP_FEE = 1e6 in your SwapMath
//                 );

//                 if (amountRemaining < 0) {
//                     // exact-in: consume input + fee; output accumulates negative
//                     amountRemaining += int256(stepIn + feeAmt);
//                     amountCalculated -= int256(stepOut);
//                 } else {
//                     // exact-out: consume desired output; input (+fee) accumulates positive
//                     amountRemaining -= int256(stepOut);
//                     amountCalculated += int256(stepIn + feeAmt);
//                 }

//                 if (sqrtPriceNext == sqrtPriceNextX96) {
//                     // crossed a tick; if initialized, update liquidity by liquidityNet
//                     if (initialized) {
//                         (, int128 liqNet) = p.ticks(tickNext);
//                         if (zeroForOne) liqNet = -liqNet; // mirror v3 sign flip when moving left
//                         L = LiquidityMath.addDelta(L, liqNet);
//                     }
//                     tick = zeroForOne ? (tickNext - 1) : tickNext;
//                 } else if (sqrtPriceNext != sqrtP) {
//                     // moved within the tick; recompute tick from price
//                     tick = TickMath.getTickAtSqrtPrice(sqrtPriceNext);
//                 }
...
//             // If we couldn't fulfill an exact-out fully, declare unavailable
//             if (exactOut && amountRemaining != 0) return (0, 0);
...
//     ) internal pure returns (bytes memory callData) {
//         callData = abi.encodeWithSelector(
//             IZRouter.swapAero.selector, to, stable, tokenIn, tokenOut, swapAmount, amountLimit, deadline
//         );
//     }
...
//             if ((!exactOut && best.amountOut == 0) || (exactOut && best.amountIn == 0)) {
//                 revert NoRoute();
//             }
...
//             } else if (best.source == AMM.AERO) {
//                 bool stable = (best.feeBps <= 2);
//                 uint256 _swapAmount = exactOut ? best.amountIn : swapAmount;
//                 uint256 _minOut = exactOut ? swapAmount : amountLimit;
//                 callData = _buildAeroSwap(to, stable, tokenIn, tokenOut, _swapAmount, _minOut, deadline);
...
//                     to, exactOut, uint24(best.feeBps * 100), tokenIn, tokenOut, swapAmount, amountLimit, deadline
//                 );
//             } else if (best.source == AMM.AERO_CL) {
//                 int24 spacing = _spacingFromBps(uint16(best.feeBps));
//                 callData = _buildAeroCLSwap(to, exactOut, spacing, tokenIn, tokenOut, swapAmount, amountLimit, deadline);
...
//                 if (best.source == AMM.AERO && exactOut) {
//                     // AERO exactOut uses exact-in underneath, needs the computed input
//                     msgValue = best.amountIn;
//                 } else {
...
//     function tickSpacingToFee(int24) external view returns (uint24); // 1e-6
//     function getSwapFee(address pool) external view returns (uint24); // 1e-6
// }

// interface IAeroV3Pool {
//     // 6 return values (sqrtPriceX96, tick, obsIndex, obsCard, obsCardNext, unlocked)
...
// function _aeroCLPoolFor(address tokenA, address tokenB, int24 spacing) view returns (address pool, bool zeroForOne) {
//     (address token0, address token1, bool zF1) = _sortTokens(tokenA, tokenB);
//     zeroForOne = zF1;
//     pool = IAeroCLFactory(AERO_CL_FACTORY).getPool(token0, token1, spacing);
// }
...
//                     int24 offset = int24(int256(uint256(lsb)) - int24(uint24(bitPos)));
//                     next = (compressed + 1 + offset) * tickSpacing;
//                 } else {
//                     next = (compressed + 1 + int24(uint24(255) - uint24(bitPos))) * tickSpacing;
//                 }
//             }
//         }
//     }
...
//     function _compress(int24 tick, int24 spacing) private pure returns (int24 compressed) {
...
// library AeroCLTickBitmap {
//     function nextInitializedTickWithinOneWord(IAeroV3Pool pool, int24 tick, int24 tickSpacing, bool zeroForOne)
//         internal
//         view
...
//     // ---- helpers ----
//     function _compress(int24 tick, int24 spacing) private pure returns (int24 comp) {
//         comp = tick / spacing;
//         if (tick < 0 && (tick % spacing != 0)) comp--;
//     }
