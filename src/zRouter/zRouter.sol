...
contract zRouter {
    ...
    function swapV4Router(bytes calldata data, uint256 deadline, uint256 ethIn, address tokenOut, uint256 amountOut)
        public
        payable
    ...
    function swapVZ(
        ...
        uint256 swapResult;
        if (!exactOut) {
            bytes4 sel = (dst == ZAMM) || (dst == CULT_HOOK) ? bytes4(0x3c5eec50) : bytes4(0x7466fde7);
            bytes memory callData = abi.encodeWithSelector(sel, key, swapAmount, amountLimit, zeroForOne, to, deadline);
            ...
            bytes4 sel = (dst == ZAMM) || (dst == CULT_HOOK) ? bytes4(0x38c3f8db) : bytes4(0xd4ff3f0e);
            bytes memory callData = abi.encodeWithSelector(sel, key, swapAmount, amountLimit, zeroForOne, to, deadline);
...
uint256 constant CULT_ID = 57896044618658097711785492504343953926636021160616296542400437774503196477768;
